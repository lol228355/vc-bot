# main.py — ВЦ 5$ бот с очередью, выплатой в канал — 100% работает на Railway 2025
import asyncio
import logging
import sqlite3
import re
...
# ==================== КЛАВИАТУРЫ (ИСПРАВЛЕНО ДЛЯ 3.14+) ====================
main_kb = ReplyKeyboardMarkup(keyboard=[
    [KeyboardButton(text="Сдать номер"), KeyboardButton(text="Где мои номера?")],
    [KeyboardButton(text="Прайс"), KeyboardButton(text="Я в сети")]
...
payout_btn = InlineKeyboardMarkup(inline_keyboard=[[
    InlineKeyboardButton(text="Забрать 5$", url=PAYOUT_CHANNEL)
]])
...
async def start(m: types.Message, state: FSMContext):
    ...
    conn = sqlite3.connect("bot.db")
    cur = conn.cursor()
    cur.execute("INSERT OR IGNORE INTO users (user_id, orders) VALUES (?, 0)", (m.from_user.id,))
...
async def ask(m: types.Message, state: FSMContext):
    if m.from_user.id in active_chats:
        return await m.answer("Ты уже в чате!")
    if not config["work"]:
        return await m.answer("Приём закрыт")
...
async def take(cb: types.CallbackQuery):
    ...
    if uid in active_chats:
        return await cb.answer("Уже взято!", show_alert=True)
...
async def admin_panel(m: types.Message):
    ...
    await m.answer("Админка", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="ВКЛ/ВЫКЛ приём", callback_data="toggle")
    ]]))
...
async def main():
    ...
    print("Бот запущен 24/7 — ВЦ 5$ — очередь работает!")
